...
import { authenticate, filterVisibleProjects, requirePermission } from '../middleware/auth.js';
const router = Router();
// 项目列表
router.get('/', authenticate, async (req, res, next) => {
    try {
        const user = req.user;
        const { status, type, search, includeArchived } = req.query;
        let projects = filterVisibleProjects(pvStore.projects, user, pvStore.tasks);
...
router.get('/:id', authenticate, async (req, res, next) => {
    try {
        const project = pvStore.projects.find((p) => p.id === req.params.id);
        if (!project)
...
// 创建项目
router.post('/', authenticate, requirePermission('project:create'), async (req, res, next) => {
    try {
        const { name, code, type, product, region, description, templateId } = req.body;
        ...
        pvStore.projects.unshift(project);
        pvStore.addAuditLog({
            id: `l-${Date.now()}`,
            actorId: user.id,
            objectType: 'PROJECT',
...
// 更新项目
router.put('/:id', authenticate, requirePermission('project:update'), async (req, res, next) => {
    try {
        const project = pvStore.projects.find((p) => p.id === req.params.id);
        if (!project)
            return res.status(404).json({ code: 404, message: 'project not found' });
        const user = req.user;
        const { name, code, type, product, region, description, memberIds } = req.body;
...
// 更新项目状态
router.put('/:id/status', authenticate, requirePermission('project:update'), async (req, res, next) => {
    try {
        const project = pvStore.projects.find((p) => p.id === req.params.id);
        if (!project)
            return res.status(404).json({ code: 404, message: 'project not found' });
