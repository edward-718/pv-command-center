...
// 创建项目
router.post('/', authenticate, requirePermission('project:create'), (req, res) => {
...
// 更新项目
router.put('/:id', authenticate, requirePermission('project:update'), (req, res) => {
    const project = pvStore.projects.find((p) => p.id === req.params.id);
    if (!project)
        return res.status(404).json({ code: 404, message: 'project not found' });
    const user = req.user;
...
// 更新项目状态
router.put('/:id/status', authenticate, requirePermission('project:update'), (req, res) => {
    const project = pvStore.projects.find((p) => p.id === req.params.id);
    if (!project)
        return res.status(404).json({ code: 404, message: 'project not found' });
    const user = req.user;
