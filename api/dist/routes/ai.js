...
function generateDraftContent(kind, projectId, store) {
    ...
    const projectTasks = Array.from(store.tasks.values()).filter((t) => t.projectId === projectId);
    ...
    const highRiskTasks = projectTasks.filter((t) => t.riskLevel === 'HIGH');
    const p0Tasks = projectTasks.filter((t) => t.priority === 'P0');
    // 按状态分布
    const statusDist = {
        '未开始': projectTasks.filter((t) => t.status === 'NOT_STARTED').length,
        '处理中': inProgressTasks,
        '待复核': inReviewTasks,
        '需补充': projectTasks.filter((t) => t.status === 'NEEDS_INFO').length,
        '已完成': doneTasks,
    };
    // 按风险等级分布
    const riskDist = {
        high: projectTasks.filter((t) => t.riskLevel === 'HIGH').length,
        medium: projectTasks.filter((t) => t.riskLevel === 'MEDIUM').length,
        low: projectTasks.filter((t) => t.riskLevel === 'LOW').length,
    };
    // 计算进度
...
**项目**: ${project?.name || projectId} ${project?.code ? `(${project.code})` : ''}
**生成时间**: ${timestamp}
...
${overdueTasks.length > 0 ? '### 逾期任务详情\n' + overdueTasks.slice(0, 5).map((t) => `- ${t.title} (逾期 ${Math.ceil((Date.now() - new Date(t.dueAt).getTime()) / (24 * 60 * 60 * 1000))} 天)`).join('\n') : ''}
...
- **高风险**: ${riskDist.high} 项
- **中风险**: ${riskDist.medium} 项
- **低风险**: ${riskDist.low} 项

## 3. 当前风险识别

### 3.1 逾期风险
${overdueTasks.length > 0 ? overdueTasks.slice(0, 5).map((t) => `- **${t.title}**\n  - 逾期天数: ${Math.ceil((Date.now() - new Date(t.dueAt).getTime()) / (24 * 60 * 60 * 1000))} 天\n  - 负责人: ${t.assigneeId}\n  - 状态: ${t.status}`).join('\n\n') : '无逾期任务。'}

### 3.2 高风险任务
${highRiskTasks.length > 0 ? highRiskTasks.slice(0, 5).map((t) => `- **${t.title}**\n  - 风险等级: ${t.riskLevel}\n  - 截止日期: ${t.dueAt}\n  - 状态: ${t.status}`).join('\n\n') : '无高风险任务。'}

### 3.3 P0 紧急任务
${p0Tasks.length > 0 ? p0Tasks.slice(0, 5).map((t) => `- **${t.title}**\n  - 截止日期: ${t.dueAt}\n  - 状态: ${t.status}`).join('\n\n') : '无 P0 紧急任务。'}

## 4. 综合风险评级

**评估**: ${overdueTasks.length > 0 || riskDist.high > 3 ? '🔴 高风险' :
