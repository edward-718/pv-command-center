...
// 创建任务
router.post('/', authenticate, requirePermission('task:update'), (req, res) => {
...
// 分配任务
router.patch('/:id/assign', authenticate, requirePermission('task:assign'), (req, res) => {
    const task = pvStore.tasks.get(req.params.id);
    if (!task)
        return res.status(404).json({ code: 404, message: 'task not found' });
    const user = req.user;
...
// 上传附件
router.post('/:id/attachments', authenticate, requirePermission('task:upload'), (req, res) => {
...
// 复核任务
router.post('/:id/review', authenticate, requirePermission('task:review'), (req, res) => {
    const task = pvStore.tasks.get(req.params.id);
    if (!task)
        return res.status(404).json({ code: 404, message: 'task not found' });
    const user = req.user;
