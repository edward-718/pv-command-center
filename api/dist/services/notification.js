 ...
 * 获取项目的提醒阈值
 */
function getReminderThresholds(projectId) {
    const project = pvStore.projects.find((p) => p.id === projectId);
    if (!project)
        return [7, 3, 1]; // 默认阈值
    const template = pvStore.templates.find((t) => t.id === project.templateId);
...
export function checkTaskDeadlines() {
    ...
    tasks.forEach((task) => {
        ...
        // 逾期通知（仅生成一次）
        if (daysUntilDue < 0) {
            const existingNotification = pvStore.notifications.find((n) => n.source === task.id && n.category === 'OVERDUE');
        ...
        thresholds.forEach((threshold) => {
            if (daysUntilDue === threshold) {
                const existingNotification = pvStore.notifications.find((n) => n.source === task.id && n.category === 'DEADLINE' && n.content.includes(`${threshold}天`));
        ...
        if (daysUntilDue === 0 && task.status !== 'IN_REVIEW') {
            const existingNotification = pvStore.notifications.find((n) => n.source === task.id && n.category === 'DEADLINE' && n.content.includes('今日到期'));
            if (!existingNotification) {
                createNotification({
                    userId: task.assigneeId,
                    source: task.id,
...
export function checkEvidenceCompleteness() {
    ...
    tasks.forEach((task) => {
        if (task.status === 'DONE' || task.status === 'IN_REVIEW')
            return;
        const taskData = pvStore.tasks.get(task.id);
        if (!taskData)
            return;
        const requiredEvidence = taskData.requiredEvidence || [];
        const evidenceUploaded = taskData.evidenceUploaded || [];
        ...
        if (missing.length > 0) {
            ...
            if (daysUntilDue <= 3 && daysUntilDue >= 0) {
                const existingNotification = pvStore.notifications.find((n) => n.source === task.id && n.category === 'EVIDENCE');
                if (!existingNotification) {
                    createNotification({
                        userId: task.assigneeId,
                        source: task.id,
 ...
 * 创建复核结果通知
 */
export function notifyReviewResult(taskId, taskTitle, decision, reason) {
...
function createNotification(params) {
    ...
    pvStore.notifications.unshift(notification);
    pvStore.save();
}
/**
 * 获取未读通知数量
...
export function getUnreadCount(userId) {
    return pvStore.notifications.filter((n) => n.userId === userId && n.status === 'UNREAD').length;
}
/**
 * 获取用户的各项通知数量
 */
export function getNotificationCounts(userId) {
    const userNotifications = pvStore.notifications.filter((n) => n.userId === userId);
    return {
