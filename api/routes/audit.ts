...
// 日志列表（支持分页、筛选）
router.get('/logs', authenticate, requirePermission('audit:read'), (req: Request, res: Response) => {
  const {
    objectType, actorId, q, projectId,
    startDate, endDate,
    page = '1', pageSize = '20'
  } = req.query as Record<string, string | undefined>;

  let list = [...(pvStore.auditLogs as Array<Record<string, unknown>>)];
  ...
  if (startDate) list = list.filter((l) => new Date(l.createdAt as string) >= new Date(startDate));
  if (endDate) list = list.filter((l) => new Date(l.createdAt as string) <= new Date(endDate));
  if (projectId) {
    list = list.filter((l) => {
      if (l.objectType === 'PROJECT' && l.objectId === projectId) return true;
      if (l.objectType === 'TASK') {
        const task = pvStore.tasks.get(l.objectId as string) as { projectId?: string } | undefined;
...
// 特定对象的审计日志
router.get('/logs/:objectType/:objectId', authenticate, requirePermission('audit:read'), (req: Request, res: Response) => {
  const { objectType, objectId } = req.params;
  const list = (pvStore.auditLogs as Array<Record<string, unknown>>)
...
// 生成并下载审计包
router.post('/export', authenticate, requirePermission('audit:export'), (req: Request, res: Response) => {
  const { projectId } = req.body as { projectId?: string };
  if (!projectId) return res.status(400).json({ code: 400, message: 'projectId required' });

  const project = (pvStore.projects as Array<Record<string, unknown>>).find((p) => p.id === projectId);
  if (!project) return res.status(404).json({ code: 404, message: 'project not found' });

  const html = generateAuditHtml(projectId, project as Record<string, unknown>);
...
// 直接下载项目审计包
router.get('/export/:projectId', authenticate, requirePermission('audit:export'), (req: Request, res: Response) => {
...
function generateAuditHtml(projectId: string, project: Record<string, unknown>): string {
  const projectLogs = (pvStore.auditLogs as Array<Record<string, unknown>>).filter((log) => {
    if (log.objectType === 'PROJECT' && log.objectId === projectId) return true;
    if (log.objectType === 'TASK') {
      const task = pvStore.tasks.get(log.objectId as string) as { projectId?: string } | undefined;
      return task?.projectId === projectId;
    }
    if (log.objectType === 'REVIEW' || log.objectType === 'ATTACHMENT') {
      const taskId = (log as { taskId?: string }).taskId;
      if (taskId) {
        const task = pvStore.tasks.get(taskId) as { projectId?: string } | undefined;
        return task?.projectId === projectId;
      }
    }
    return false;
  ...
  projectLogs.forEach((l) => {
    if (l.objectType in statusCounts) statusCounts[l.objectType as keyof typeof statusCounts]++;
  });

  const logsHtml = projectLogs.map((log) => {
    const typeLabel = { TASK: '任务', PROJECT: '项目', REVIEW: '复核', ATTACHMENT: '附件', EXPORT: '导出' }[log.objectType as string] || log.objectType;
    return `<tr>
      <td>${new Date(log.createdAt as string).toLocaleString('zh-CN')}</td>
...
<title>审计包 · ${(project as { code?: string }).code || projectId}</title>
<style>
...
<div class="meta">
<div class="meta-item"><label>项目编号</label><span>${(project as { code?: string }).code || '-'}</span></div>
