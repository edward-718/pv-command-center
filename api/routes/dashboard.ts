...
router.get('/', authenticate, (req: Request, res: Response) => {
  ...
  const visibleTasks = tasks.filter((t) => {
    ...
    if (user.role === 'VENDOR') return t.assigneeId === user.id;
    return true;
  });
  ...
  // 项目统计
  const projects = pvStore.projects as Array<{ id: string; status: string; type: string }>;
...
router.get('/kpis', authenticate, (req: Request, res: Response) => {
  ...
  // 过程指标
  const submittedForReview = visibleTasks.filter((t) =>
    ['IN_REVIEW', 'NEEDS_INFO', 'DONE'].includes(t.status)
  );
