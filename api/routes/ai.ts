...
${riskDist['高'] > 0 ? `⚠️ 高风险任务: ${riskDist['高']} 项\n` : ''}${overdueTasks.length > 0 ? `⚠️ 逾期任务: ${overdueTasks.length} 项\n` : ''}${p0Tasks.length > 0 ? `🔴 P0 紧急任务: ${p0Tasks.length} 项\n` : ''}${riskDist['高'] === 0 && overdueTasks.length === 0 && p0Tasks.length === 0 ? '✅ 无高风险/逾期/P0 任务\n' : ''}
...
**评估**: ${
  overdueTasks.length > 0 || riskDist['高'] > 3 ? '🔴 高风险' :
  overdueTasks.length > 0 || riskDist['高'] > 0 ? '🟡 中风险' :
  '🟢 低风险'
}
