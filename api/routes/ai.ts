...
function generateDraftContent(kind: string, projectId: string, store: typeof pvStore): string {
  ...
  // 按风险等级分布
  const riskDist = {
    high: projectTasks.filter((t) => (t as { riskLevel: string }).riskLevel === 'HIGH').length,
    medium: projectTasks.filter((t) => (t as { riskLevel: string }).riskLevel === 'MEDIUM').length,
    low: projectTasks.filter((t) => (t as { riskLevel: string }).riskLevel === 'LOW').length,
  };
...
**项目**: ${(project as { name?: string })?.name || projectId} ${(project as { code?: string })?.code ? `(${(project as { code?: string }).code})` : ''}
**生成时间**: ${timestamp}
